...
Generate a population JSON template

## Synopsis

Generate a JSON skeleton template for population create or replace bodies

```
pingcli pingone populations template [flags]
```
...
# Generate a template and save to a file
  pingcli pingone populations template > body.json

  # Edit the template, then create the population
  pingcli pingone populations create --environment-id <env-id> --from-file body.json
...
Example workflow:
  pingcli pingone populations template > body.json
  # edit body.json
  pingcli pingone populations create --from-file body.json
```
