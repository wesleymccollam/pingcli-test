# `pingcli pingone sign-on-policies template`
Generate a sign-on policy JSON template

## Synopsis

Generate a JSON skeleton template for sign-on policy create or replace bodies

```
pingcli pingone sign-on-policies template [flags]
```
...
# Generate a template and save to a file
  pingcli pingone sign-on-policies template > body.json

  # Edit the template, then create the sign-on policy
  pingcli pingone sign-on-policies create --environment-id <env-id> --from-file body.json
...
Example workflow:
  pingcli pingone sign-on-policies template > body.json
  # edit body.json
  pingcli pingone sign-on-policies create --from-file body.json
```
