# `pingcli pingone davinci variables template`
Generate a DaVinci variable JSON template

## Synopsis

Generate a JSON skeleton template for DaVinci variable create or replace bodies

```
pingcli pingone davinci variables template [flags]
```
...
# Generate a template and save to a file
  pingcli pingone davinci variables template > body.json

  # Edit the template, then create the variable
  pingcli pingone davinci variables create --environment-id <env-id> --from-file body.json
...
Example workflow:
  pingcli pingone davinci variables template > body.json
  # edit body.json
  pingcli pingone davinci variables create --from-file body.json
```
