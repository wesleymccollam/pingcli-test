...
# Generate a template and save to a file
  pingcli pingone groups template > body.json

  # Edit the template, then create the group
  pingcli pingone groups create --environment-id <env-id> --from-file body.json
...
Example workflow:
  pingcli pingone groups template > body.json
  # edit body.json
  pingcli pingone groups create --from-file body.json
```
