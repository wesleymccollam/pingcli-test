# `pingcli authorize application-resource-permissions replace`
...
```
pingcli authorize application-resource-permissions replace [flags]
```
...
# Update an application resource permission from a JSON file (path flags are still required)
  pingcli authorize application-resource-permissions replace --environment-id <env-id> --application-resource-id <app-resource-id> --application-resource-permission-id <permission-id> --from-file permission.json
