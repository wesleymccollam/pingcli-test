# `pingcli davinci flows template`
...
```
pingcli davinci flows template [flags]
```
...
# Generate a template and save to a file
  pingcli davinci flows template > body.json

  # Edit the template, then create the flow
  pingcli davinci flows create --environment-id <env-id> --from-file body.json
...
Example workflow:
  pingcli davinci flows template > body.json
  # edit body.json
  pingcli davinci flows create --from-file body.json
```
