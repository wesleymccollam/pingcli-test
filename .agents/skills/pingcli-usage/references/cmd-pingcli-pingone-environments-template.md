# `pingcli pingone environments template`
Generate an environment JSON template

## Synopsis

Generate a JSON skeleton template for environment create or replace bodies

```
pingcli pingone environments template [flags]
```
...
# Generate a template and save to a file
  pingcli pingone environments template > body.json

  # Edit the template, then create the environment
  pingcli pingone environments create --from-file body.json
