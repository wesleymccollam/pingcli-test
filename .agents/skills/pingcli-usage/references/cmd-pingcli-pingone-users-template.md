# `pingcli pingone users template`
Generate a user JSON template

## Synopsis

Generate a JSON skeleton template for user create or replace bodies

```
pingcli pingone users template [flags]
```
...
# Generate a template and save to a file
  pingcli pingone users template > body.json

  # Edit the template, then create the user
  pingcli pingone users create --environment-id <env-id> --from-file body.json
...
Example workflow:
  pingcli pingone users template > body.json
  # edit body.json
  pingcli pingone users create --from-file body.json
```
