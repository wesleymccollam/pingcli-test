# `pingcli pingone authorize api-servers template`
Generate an API server JSON template

## Synopsis

Generate a JSON skeleton template for API server create or replace bodies

```
pingcli pingone authorize api-servers template [flags]
```
...
# Generate a template and save to a file
  pingcli pingone authorize api-servers template > body.json

  # Edit the template, then create the API server
  pingcli pingone authorize api-servers create --environment-id <env-id> --from-file body.json
...
Example workflow:
  pingcli pingone authorize api-servers template > body.json
  # edit body.json
  pingcli pingone authorize api-servers create --from-file body.json
```
