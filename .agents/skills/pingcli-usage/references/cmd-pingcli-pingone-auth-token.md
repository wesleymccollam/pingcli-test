# `pingcli pingone auth token`
Print the active token for the current authenticated session.
...
```
pingcli pingone auth token [flags]
```

## Examples

```
# Print the active access token for the current PingOne session.
    pingcli pingone auth token
```
