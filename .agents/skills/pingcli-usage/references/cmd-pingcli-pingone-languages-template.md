# `pingcli pingone languages template`
Generate a language JSON template

## Synopsis

Generate a JSON skeleton template for language create or replace bodies

```
pingcli pingone languages template [flags]
```
...
# Generate a template and save to a file
  pingcli pingone languages template > body.json

  # Edit the template, then create the language
  pingcli pingone languages create --environment-id <env-id> --from-file body.json
...
Example workflow:
  pingcli pingone languages template > body.json
  # edit body.json
  pingcli pingone languages create --from-file body.json
```
