# `pingcli pingone sign-on-policies sop-actions replace`
...
```
pingcli pingone sign-on-policies sop-actions replace [flags]
```
...
# Update a sign-on policy action from a JSON file (SignOnPolicyActionBody wrapper shape: one nested type block)
  pingcli pingone sign-on-policies sop-actions replace --environment-id <env-id> --sign-on-policy-id <sop-id> --sign-on-policy-action-id <action-id> --from-file action.json
