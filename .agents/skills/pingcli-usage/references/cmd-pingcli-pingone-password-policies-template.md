# `pingcli pingone password-policies template`
Generate a password policy JSON template

## Synopsis

Generate a JSON skeleton template for password policy create or replace bodies

```
pingcli pingone password-policies template [flags]
```
...
# Generate a template and save to a file
  pingcli pingone password-policies template > body.json

  # Edit the template, then create the password policy
  pingcli pingone password-policies create --environment-id <env-id> --from-file body.json
...
Example workflow:
  pingcli pingone password-policies template > body.json
  # edit body.json
  pingcli pingone password-policies create --from-file body.json
```
